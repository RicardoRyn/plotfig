...
def _check_surf(surf):
    ...
    if isinstance(surf, (str, pathlib.Path)):
        ...
        raise ValueError(
            "Surface be a path-like string, an instance of BSPolyData, or None"
        )
...
def _check_data(data):
    ...
    if isinstance(data, np.ndarray):
        ...
        raise TypeError(
            "data must be a file path to a valid GIFTI or CIFTI "
...
class Plot(object):
    ...
    def __init__(
        ...
        self.surfaces = {k: _check_surf(v) for k, v in hemi_inputs if v is not None}
        ...
        self.plot_layout = _set_layout(surf_lh, surf_rh, layout, views, mirror_views)
        self.flip = flip
    ...
    def add_layer(
        ...
        if isinstance(data, valid_types):
            ...
            if set(data.keys()) <= set(["left", "right"]):
                ...
                raise ValueError("Only valid keys for `data` are 'left' and/or 'right'")
        else:
        ...
        for k, v in self.surfaces.items():
            if k in vertices.keys():
                if as_outline:
                    x = get_labeling_border(v, np.nan_to_num(vertices[k])).astype(float)
                else:
                    x = vertices[k]
                if zero_transparent:
                    x[x == 0] = np.nan
    ...
    def render(self, offscreen=True, try_qt=False):
        ...
        if self.flip and len(self.surfaces) == 2:
            view_layout, hemi_layout = _flip_hemispheres(view_layout, hemi_layout)
    ...
    def _add_colorbars(
        ...
        """
        cbar_pads = [0.01] + [pad] * (len(self._show_cbar) - 1)
